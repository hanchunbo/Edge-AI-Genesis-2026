...
**严重度**：🟢 低（属于 P1/P2 范围，不阻塞核心闭环）

**现状**：Qt6 对开发者而言是新技术栈，W22-W25 共 4 周需完成：
Qt 机制学习 + Worker Thread + 60FPS 渲染 + 双平台打包，
伙伴可协助但不能代替核心推理线程设计。

**修复方向**：建议弹性执行策略：
1. W22-W23 优先出可演示最小 GUI（摄像头画面 + 检测框叠加），作为演示物
2. W24-W25 根据剩余时间决定做深（复杂 UI / 打包）还是转向文档完善
3. EdgeSight 完整版（W26）定位为 bonus，不作为硬门槛
