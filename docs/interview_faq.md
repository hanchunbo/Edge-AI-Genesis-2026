...
## W16：YOLOv8 检测 demo 源码精读（坐标系 / 命名空间 / NCHW / 授权）

### Q43: YOLO 解码里框的左上角是 `y = cy - h/2`，为什么 y 用减法而不是加法？

**考察点**：图像坐标系与数学坐标系的差异——CV 里反复出现的坑。

**答题角度**（概念正文见主题库 [image-ops.md — 图像坐标系（y 轴朝下）](notes/image-ops.md#图像坐标系y-轴朝下--cxcywhxyxy--clamp-夹边界)）：
图像坐标系原点在**左上角**、**y 向下增大**（与数学课 y 向上相反，因为图片从上到下逐行存）。所以「上边缘」y **更小** → 左上角 `y = cy - h/2`（减法）。x 的左右与直觉一致（左减右加），唯独 y 的上下翻转。规律记成「左上角 = 两个都取小（都减），右下角 = 两个都取大（都加）」。

**加分回答**：
> "反算回原图后还要 `std::clamp(v, 0, 图宽/高)` 夹边界——贴边目标反算出负坐标或超界值，不夹会让 OpenCV 越界崩溃、NMS 面积算错。同一『几何量夹回合法区间』的思想在 IoU 算交集时也有：无交集时负宽×负高会得假正面积，要先 `max(0, ...)`。"

---

### Q44: `yolo_detector.cpp` 用 `namespace w16`，而 `yolo_demo.cpp` 用匿名 `namespace {}`，为什么不一样？

**考察点**：具名 vs 匿名命名空间的用途、内部链接、库代码 vs 可执行入口。

**答题角度**（概念正文见主题库 [cpp-core.md — 匿名命名空间 vs 具名命名空间](notes/cpp-core.md#匿名命名空间-vs-具名命名空间)）：
判据是「要不要被别的文件引用」。detector 是**库代码**，类型要给 demo/test include 用，名字（`w16::`）就是 API 契约 → 具名；项目约定每模块独立空间（`w1`…`w16`）避免跨模块同名。demo 是 `main` 所在的**叶子文件**，没人 include，辅助函数只自己用 → 匿名命名空间（内部链接，等价文件级 `static`），不暴露符号、避免链接冲突。

**加分回答**：
> "`.hpp` 和 `.cpp` 里那两个 `namespace w16` 不是两个命名空间，是同一个分两处书写。另外匿名命名空间是现代 C++ 替代『文件级 static 函数』的写法，还能装类型和模板，static 不能。"

---

### Q45: 输入 shape `{1, 3, 640, 640}` 第一个 `1` 是什么？只推一张图为什么不能省略？

**考察点**：NCHW 四维语义、batch 维、张量 buffer 与 shape 自洽。

**答题角度**（概念正文见主题库 [inference.md — NCHW 四维含义](notes/inference.md#nchw-四维含义batch--channel--h--w)）：
NCHW = batch / channel / height / width，第一个 `1` 是 **batch 维**（这次喂几张图）。维度是模型 I/O 的固定契约，哪怕一张也要显式写出 batch 维。`1×3×640×640` 正好等于张量 buffer 的元素数；那个 `1` 告诉引擎「这坨数据是 1 张图、batch 在最外层」。输出端对称校验 `out_shape[0]==1`。

**加分回答**：
> "batch 是可调旋钮——批量推理提吞吐时改成 N 并在最外层拼 N 张图（W16 benchmark 的 batch 1v4）。还要注意 NCHW 通道在外、NHWC（TF 默认）通道在内，别搞混内存排布。顺带 YOLOv8 输出 `[1, C, A]` 的 C=4+类别数、A=anchor 数，且是 channels-major 布局，解码要按 `out[ch*A+a]` 跨步取。"

---

### Q46: 项目里用的 yolov8n 是开源免费的吗？商用有什么要注意的？

**考察点**：开源协议意识、AGPL 传染性、工程合规（面试体现成熟度的加分项）。

**答题角度**：
Ultralytics 的 YOLOv8 代码和权重**免费公开**，但许可证是 **AGPL-3.0**——传染性（copyleft）很强的协议：自己学习/研究/放公开仓完全免费；但一旦用进**对外提供服务的产品**（哪怕只是部署成网络 API），AGPL 要求把整个产品源码也按 AGPL 开源。Ultralytics 靠这个做「开源 + 商业双授权」，不想开源就买 Enterprise 商业授权。

**加分回答**：
> "所以闭源商业产品要么买授权，要么换 Apache-2.0 的检测器（如旷视 YOLOX）规避。还有个细节：本仓根 LICENSE 是 MIT，却引入了 AGPL 权重——严格说『MIT 仓依赖 AGPL 组件』在协议兼容性上有争议，学习 demo 无所谓，但知道这点能体现懂工程合规。注意 YOLO 各版本协议不同：v1–v4(Darknet) 宽松、YOLOX 是 Apache、v8/v10 系基本 AGPL。"
...
**加分回答**：
> "Entropy 不是一定更准。它更慢、更吃内存，而且如果被 clip 的少数极端值对模型很关键，也会伤精度。还有实现坑：本项目实测 ORT `quantize_static` 默认参数下 Entropy 直接退化成 MinMax——两个 INT8 产物字节级相同（机制见主题库坑③与 Q59）——所以对比实验前要先确认两条路线产出真的不同。"
...
### Q59: 两组对比实验的指标一位小数都不差，你的第一反应是什么？（实战：ORT Entropy 校准退化）

**考察点**：取证优先的实验素养；是否踩过并能讲清 ORT 量化实现的真实坑。

**答题角度**（概念正文见主题库 [inference.md — PTQ / MinMax / Entropy](notes/inference.md#ptq--minmax--entropy) 坑③）：
先怀疑「根本是同一个产物」而不是「方法恰好等价」——第一步 `sha256sum` 对比产物文件（或 diff 原始输出），10 秒排除或坐实。本项目实战：MinMax/Entropy 两个 INT8 ONNX 的 coco128 mAP 完全相同，哈希取证发现文件字节级相同；根因是 ORT `quantize_static` 默认 `num_bins=128` 与 `num_quantized_bins=128` 相等，KL 候选窗口只剩「全范围」1 个，选出的对称全范围再被钳回真实 min/max——与 MinMax 逐比特一致；且 `extra_options` 不透传 `num_bins`，无法用参数修正。

**加分回答**：
> "排查用了三级取证：产物哈希 → 8 图小规模复现（排除偶发）→ 绕过 quantize_static 直接插桩两个校准器对比 292 层 range（逐比特相同，坐实覆盖发生在校准器内部）。结论对选型反而更有利：Entropy 白吃直方图内存产出一样的东西，选 MinMax 理由更硬。想要真正的 KL 校准得换实现，比如 TensorRT 的 IInt8EntropyCalibrator2 用 2048 bins，有真实搜索空间。"

---

### Q60: Entropy(KL) 校准里有哪两种「格子」？KL 散度算的是相似度吗？

**考察点**：是否真懂 KL 校准算法内部机制，而不是背「histogram + KL」一句话。

**答题角度**（概念正文见主题库 [inference.md — PTQ / MinMax / Entropy](notes/inference.md#ptq--minmax--entropy)）：
两种格子：直方图格数（`num_bins`）是**测量分辨率**，自选尺子，TRT 用 2048；量化格数（`num_quantized_bins`≈128）模拟**对称 INT8 的单侧级数**（256 的一半），是客观常数。候选截断窗口从直方图中心对称外扩、宽度至少「量化格数」格，候选数 = (num_bins − num_quantized_bins)/2 + 1。每个候选造两个分布：P = 真实（窗口内容 + 窗外值堆两端），Q = INT8 近似（窗口并成 128 格再摊平），KL(P‖Q) 打分选最低。KL 是**差异度**不是相似度（0 = 完全一致，越大越差）；且**不对称**——站在真实分布 P 的立场问「拿 Q 冒充 P 要付多少信息代价」，按 P 的占比加权：真实值密集的区间失真罚得重。

**加分回答**：
> "窄窗口罚截断（P 两端堆了被钳的离群值）、宽窗口罚粗步长（Q 把主体分布糊成大色块），但最优不一定在中间——尾巴稀时最窄窗口反而赢，完全由每层分布形状决定。KL 学名相对熵（relative entropy），这就是 Entropy 校准名字的来源。"
