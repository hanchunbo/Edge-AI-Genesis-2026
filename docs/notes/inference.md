...
**Entropy**：先收集 activation histogram，再尝试多个候选 clipping 阈值；每个阈值下模拟量化/反量化，把量化后的分布 `Q` 与原始分布 `P` 做 KL Divergence（relative entropy）比较，选择信息损失最小的范围。它愿意牺牲少数极端值，换取主体分布更细的量化步长；代价是更慢、更吃内存，且若极端值本身重要也会伤精度。实现层面要分清两种「格子」：直方图格数（`num_bins`）是测量分辨率、可自选（TRT 用 2048）；量化格数（`num_quantized_bins`≈128）模拟对称 INT8 的单侧级数、是客观常数——候选截断窗口从直方图中心对称外扩、宽度至少「量化格数」格，**候选数 = (num_bins − num_quantized_bins)/2 + 1**。P = 真实（窗口内容 + 窗外值堆两端），Q = INT8 近似（窗口并成 128 格再摊平）：窄窗口罚截断（P 两端堆）、宽窗口罚粗步长（Q 中间糊），最优不必在中间（尾巴稀时最窄窗口赢）。KL 是差异度而非相似度（0 = 完全一致），且不对称：站在真实分布 P 的立场问「拿 INT8 近似 Q 冒充 P 要付多少信息代价」，按 P 的占比加权罚分。
