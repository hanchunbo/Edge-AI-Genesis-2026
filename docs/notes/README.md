...
| 文件 | 收纳主题 | 来源周 | 状态 |
|---|---|---|---|
| [inference.md](inference.md) | ONNX vs ORT、shape/tensor/Ort::Value、Env/Session/Engine、Env 全局唯一、零拷贝输入、CPU/GPU 内存 + Host→Device、I/O 元数据缓存、ORT Run name 绑定、EP 优雅回退 + ActiveEp≠全图(混合执行)、Softmax、Top-K + argmax 退化、NCHW 四维(batch 维)、检测术语(Ultralytics/score/thresh/anchor)、YOLOv8 检测头(无 objectness/转置)、Backbone/Neck/Detection Head、NMS/IoU(逐类)、IntraOp vs InterOp、IOBinding、FP32/FP16/INT8、量化范围/scale/zero_point/clipping、PTQ/MinMax/Entropy、对称/非对称、Per-Tensor/Per-Channel、QDQ/QOperator、量化 vs 剪枝 vs 蒸馏 | W14/W15/W16/quant | ✅ W14 + W15 后处理 + W16 检测/线程/IOBinding + quant PTQ 概念已入 |
...
| [systems-perf.md](systems-perf.md) | P50/P99、warmup/iters、吞吐 img/s vs FPS、micro-benchmark vs end-to-end、部署硬化 vs 后端优化、Benchmark/EvalHarness、profiling 报告方法、fallback/Debug/skip 的结论边界；mmap/perf/Valgrind/GDB 待补 | W11/W16/quant | 🟡 部分（W16 benchmark + quant profiling 方法已入） |
