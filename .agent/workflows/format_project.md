...
**适用范围**：仅 `01_Linux_CPP_Foundations/`（w1–w11、w13）与 `02_Inference_Analysis/w14`、`w15` 存档模块。W16 起的模块**跳过本节**，按 `CLAUDE.md`「Documentation & Comments」节执行（Doxygen `///` brief + `@pre`/`@warning`/`@note` 坑标签）。
