...
**当前交付物：`quant`（Phase 0）进行中——部署硬化（W16 加性扩展）+ 评估 harness + ORT static INT8 PTQ 工具链已落地；遗留：INT8 精度失败样本（单图校准 → 0 检测框，需扩校准集/排除检测头敏感节点）、mAP 评估未做、IOBinding 双绑 + buffer 池未做、本机 CPU ORT 包致 CUDA EP 回退（GPU 数字待重测）。细节见 `02_Inference_Analysis/quantization/notes.md` 与 `docs/benchmarks/quant_*.md`。W1–W16 全部完成（W16 YOLOv8n 检测 Demo 为 quant 基线），遗留 ResNet18 对比（降级可选）、VPS CPU EP 环境（待定）。**
