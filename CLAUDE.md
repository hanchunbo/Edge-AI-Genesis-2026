...
**当前交付物：`quant`（Phase 0）进行中——部署硬化（W16 加性扩展）+ 评估 harness + ORT static INT8 PTQ 工具链已落地；INT8 0 检测框已修复（检测头 /model.22/ 保 FP32 + coco128 校准，`Quant_Int8ConsistencyTest` 框级对齐守护）；coco128 mAP 掉点评估已跑通（INT8 mAP50-95 0.4285 vs FP32 0.4454，掉约 1.7 点，精度可用）。遗留：IOBinding 双绑 + buffer 池未做、本机 CPU ORT 包致 CUDA EP 回退（GPU 数字待重测）。细节见 `02_Inference_Analysis/quantization/notes.md` 与 `docs/benchmarks/quant_*.md`。W1–W16 全部完成（W16 YOLOv8n 检测 Demo 为 quant 基线），遗留 ResNet18 对比（降级可选）、VPS CPU EP 环境（待定）。**
