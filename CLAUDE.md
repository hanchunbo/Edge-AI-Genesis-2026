...
## 理解确认（Understanding Check）

本仓库是学习型项目，产出代码之外更要产出理解。以下两种场景，Claude 在收尾时必须向用户提出 2~3 个针对性问题，确认用户真正掌握：

1. **方案被修改后**：凡 spec / 里程碑 / 关键决策 / 重要结论发生变更（无论谁提出），提问覆盖「改了什么、为什么这么改、影响到哪些后续工作」。
2. **用户说「看完了」某模块内容或代码后**：提问覆盖核心机制与设计决策，优先问 why（为什么这里用 X 而不是 Y）而不是 what（这行代码做了什么）。

回答处理：答对 → 简短确认，不啰嗦；答错或含糊 → 当场讲解，并指向对应模块笔记 / 主题库条目。
