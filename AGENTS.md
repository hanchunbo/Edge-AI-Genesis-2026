...
> 本文件是给 Codex / Claude Code 等 AI coding agents 的项目说明书。
> AI 助手每次进入项目时应读取此文件，按照这里的规范来写代码、命名、注释和提交。

## 当前进度

**当前交付物：`quant`（Phase 0）进行中——部署硬化（W16 加性扩展）+ 评估 harness + ORT static INT8 PTQ 工具链已落地；INT8 0 检测框已修复（检测头 /model.22/ 保 FP32 + coco128 校准，`Quant_Int8ConsistencyTest` 框级对齐守护）。遗留：mAP 掉点评估未做、INT8 报告数字待重跑刷新、IOBinding 双绑 + buffer 池未做、本机 CPU ORT 包致 CUDA EP 回退（GPU 数字待重测）。W16 YOLOv8n 检测 Demo 已作为基线完成，遗留 ResNet18 对比（降级可选）、VPS CPU EP 环境（待定）。**
...
- **函数头（语法用行业标准，不自造格式）**：
  - C++：Doxygen `///`。一句话 brief 必写（类 + 非平凡函数，含 cpp 内部）；
    ...
    声明与定义分离时，brief 只写在**声明侧**（.hpp）；纯 .cpp 内部函数（匿名命名空间 /
    static）写在定义侧。同一函数不写两份——与下面的位置绑定原则一致。
...
- 示例：
  ...
  /// @warning out 指向 session 内部缓冲，下次 Run() 后失效
  [[nodiscard]] double RunOnce(const cv::Mat& img, std::span<Detection>& out);

  // 阈值取 3.0 而非 1.0：实测 P50 多轮间会正负翻转，1% 会把抖动判成收益
  //（基准方法见 notes.md §噪声判定）
...
`[Legacy]/[Pain Point]/[Modern]` 三段式仅存在于 W1–W15 存档模块。W16 起的模块
**不再使用**此格式；若确需说明新旧取舍，一行「为什么用 X 而非 Y」写进意图注释即可。

存档模块本身冻结不改，仅允许向后兼容的加性扩展；扩展时沿用该模块既有的演进式风格
（流程见 `.agent/workflows/format_project.md` §4），保持存档内部一致。若该模块本就没有
演进式注释（约半数存档模块如此），则不必新增，按上面的新标准写即可。

注：目录结构与测试命名的存档边界是 **W1–W16**（见上文「结构切换」），注释风格的边界是
**W1–W15**——两者维度不同，不是笔误：W16 已基本不用演进式注释，是新风格的天然起点。
...
**铁律：Codex / Claude Code 不能未经用户明确许可就把任何改动合入 `main`。**
