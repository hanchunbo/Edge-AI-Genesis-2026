...
图像缩放时，目标像素的颜色值需要从原图"估算"出来，这就是插值。

**最近邻（Nearest Neighbor）**：直接取原图中距离最近的那个像素的值。
- 速度最快，但边缘锯齿明显（像素块感强）
- 适合：速度优先、或不能模糊边缘（如语义分割 mask）

**双线性（Bilinear）**：取周围 4 个像素，按距离加权平均。
- 稍慢，但过渡平滑
- 适合：普通图像缩放、推理前处理的默认选择

```
最近邻：新点 → 取最近的 1 个像素
双线性：新点 → 对周围 2×2 共 4 个像素做加权平均
```

### 双线性公式直觉

`wx`、`wy` 是新像素落点的小数部分（偏离左/上方像素的距离，范围 0~1）：
...
```
// 第一步：水平方向各插一次
top_row    = (1-wx)*P00 + wx*P10   // 上边那行的估算值
bottom_row = (1-wx)*P01 + wx*P11   // 下边那行的估算值

// 第二步：垂直方向合并
result = (1-wy)*top_row + wy*bottom_row
```

直觉：距离越近权重越大，wx=0 完全取左边，wy=1 完全取下边。

### Q8 定点：两个常见困惑

**`+ 0.5` 的意图**：这是整数四舍五入的标准写法，`int(x + 0.5)` 等价于 `round(x)`。
`int()` 直接截断会系统性偏低（如 `0.996×256=254.976` 截断到 254），加 0.5 后进位到 255。

**`>> 16` 为什么能还原**：两次 `×256` 放大了 `256×256=65536` 倍，`>> 16` 就是 `÷65536`，正好抵消：
...
### 推理引擎是什么（ONNX Runtime / TensorRT）

模型训练完之后，要在设备上做预测，这个"跑"的过程叫**推理（Inference）**。推理引擎专门负责高效执行：

| 引擎 | 厂商 | 擅长 |
|------|------|------|
| ONNX Runtime | Microsoft | 跨平台通用，支持 CPU/GPU，接受 `.onnx` 格式 |
| TensorRT | NVIDIA | 专为 NVIDIA GPU 极致优化，做量化/层融合，延迟最低 |

类比：训练完的模型是"食谱"，推理引擎是"厨师"——TensorRT 是米其林大厨（快但只用 NVIDIA 灶台），ONNX Runtime 是万能厨师（慢一点但哪都能用）。

W10 的 `LetterboxToTensor` 输出的 `std::vector<float>` 就是喂给推理引擎的输入格式。
