...
void use_buffer_by_ref(const TensorBufferPtr &ptr) {
  ...
  std::cout << "  [use_buffer_by_ref] 引用计数（未增加）: " << ptr.use_count()
            << std::endl;
}

void use_buffer_by_value(TensorBufferPtr ptr) {
  // 值传递，引用计数 +1
  std::cout << "  [use_buffer_by_value] 引用计数（+1）: " << ptr.use_count()
            << std::endl;
}
...
void test_weak_ptr() {
...

  {
    ...
    weak = shared;
    std::cout << "weak = shared，引用计数（不变）: " << shared.use_count()
              << std::endl;
    std::cout << "weak.expired(): " << (weak.expired() ? "是" : "否")
              << std::endl;

    // 使用 lock() 获取 shared_ptr
    if (TensorBufferPtr locked = weak.lock()) {
      std::cout << "weak.lock() 成功，引用计数: " << locked.use_count()
                << std::endl;
    }
...
void test_vector_move() {
  ...
  // valid()检查对象是否“有效”
  std::cout << "\ntemp.valid() after move: " << (temp.valid() ? "是" : "否")
            << std::endl;
