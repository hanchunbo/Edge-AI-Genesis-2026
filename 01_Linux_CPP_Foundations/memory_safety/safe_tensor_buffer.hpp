...
public:
    ...
    explicit SafeTensorBuffer(size_t size) : size_(size), data_(nullptr) {
        ...
        std::cout << "[SafeTensorBuffer] 构造: 分配 " << size << " 字节" << std::endl;
        ...
        std::cout << "[SafeTensorBuffer] 地址 = " << static_cast<void*>(data_) << std::endl;
    }
    
    /// 析构函数 - 自动释放内存
    ~SafeTensorBuffer() {
        if (data_ != nullptr) {
            std::cout << "[SafeTensorBuffer] 析构: 释放 " << size_ << " 字节" << std::endl;
            delete[] data_;
            data_ = nullptr;
        } else {
...
using UniqueTensorBuffer = std::unique_ptr<SafeTensorBuffer, TensorBufferDeleter>;
